...
### Fixed
- Transcript extraction broken by YouTube's migration from Polymer `ytd-transcript-segment-renderer` to new `transcript-segment-view-model` components (Feb 2026)
- Updated primary DOM selectors: `transcript-segment-view-model`, `.ytwTranscriptSegmentViewModelTimestamp`, `.yt-core-attributed-string`
- Legacy Polymer selectors kept as automatic fallback for older YouTube UI versions
